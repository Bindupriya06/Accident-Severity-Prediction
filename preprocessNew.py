...
class prediction(object):
    ...
    def preprocess(self):
       ...
       # self.df.drop(self.df.loc[self.df['Urban_or_Rural_Area']=='Unallocated'].index, inplace=True)
        ...
        accident_severity = {"Accident_Severity": {"Serious": 1, "Fatal": 2, "Slight": 0}}
    ...
    def printAnalysis(self, actual, pred):
        print("accuracy_score for : %f" %round(accuracy_score(actual, pred)*100,2))
    ...
    def __init__(self):
        ...
        filename = os.path.join(dirname, 'data/dataset.csv')
